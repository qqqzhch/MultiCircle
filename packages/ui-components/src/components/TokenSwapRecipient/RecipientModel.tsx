...
export default function RecipientModel() {
  ...
  if(selectCheck==false){
   ...
   setCustomRecipientAddress(recipientAddress)
   setIsOpen(false)

  },[recipientAddress,addToast,selectCheck,setCustomRecipientAddress,setIsOpen])
    ...
    <>
      <div className="flex items-center font-medium">
        <button onClick={openModal} className="text-valuerouter-theme-primary flex items-center font-semibold">
          <If condition={CustomRecipientAddress==null}>
           <Then>
          ...
          <span className="ml-1">Add Address</span>
           ...
           <Else>
           <span className=' font-normal'>{CustomRecipientAddress&&cutOut(CustomRecipientAddress,4,4)}</span> 
